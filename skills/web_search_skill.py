...
class WebSearchSkill(BaseSkill):
    ...
    def _score_result(cls, query: str, result: dict) -> float:
        ...
        official_domains = {"python.org": ("python",), "docs.python.org": ("python",), "pypi.org": ("python",), "github.com": ()}
    ...
    def _normalize_query(query: str) -> str:
        q = query.strip()
        low = q.lower()
        if "python" in low and any(k in low for k in ("latest", "current", "release", "version")):
            return "site:python.org latest Python release downloads"
        return q
        ...
        for provider in self.providers:
            ...
            try:
                ...
                output = [
                    ...
                    "Evidence is source material only. Final answers must not add unsupported facts.\n",
                ]
                for index, result in enumerate(ranked[:max_results], start=1):
                    title, href, body = result.get("title", ""), result.get("href", ""), result.get("body", "")
                    ...
                    if verify and index <= verify_limit and href:
                        output.append(f"VERIFIED_PAGE_TEXT: {self._verify_url(href)}\n")
                return "\n".join(output)
            except Exception as exc:
